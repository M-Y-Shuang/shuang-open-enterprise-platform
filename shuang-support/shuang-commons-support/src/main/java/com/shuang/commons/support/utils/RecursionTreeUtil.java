package com.shuang.commons.support.utils;

import com.shuang.commons.support.bean.TreeNode;
...
public class RecursionTreeUtil {
	...
	private static void recursionFn(List<TreeNode> list, TreeNode node) {
		...
		if (com.shuang.commons.support.utils.ObjectUtil.isEmpty(childList)) {
			return;
		}
	...
	private static List<TreeNode> getChildList(List<TreeNode> list, TreeNode t) {
		...
		for (TreeNode treeNode : list) {
			if (com.shuang.commons.support.utils.ObjectUtil.isEmpty(treeNode.getPid())) {
				continue;
			}
